...
public sealed class ItemStoragePanelView : MonoBehaviour
{
    ...
    public void ShowItemDetails(ItemRuntimeData item)
    {
        ...
        {
            _detailsText.text = BuildItemDetailsText(definition);
        }
    }
    ...
    private string BuildItemDetailsText(ItemDefinition item)
    {
        StringBuilder builder = new StringBuilder();
        builder.AppendLine($"Тип: {GetGroupDisplayName(item.Group)}");
        builder.AppendLine();
        builder.AppendLine("Модификаторы:");
        int modifiersCount = AppendModifierGroup(builder, item, ItemStatTarget.Primary, "Основные статы:");
        modifiersCount += AppendModifierGroup(builder, item, ItemStatTarget.Secondary, "Вторичные статы:");
    ...
    private int AppendModifierGroup(StringBuilder builder, ItemDefinition item, ItemStatTarget target, string title)
    {
    ...
    private string FormatModifier(ItemStatModifier modifier)
    {
        ...
        string statName = GetModifierStatName(modifier);
        string suffix = GetModifierValueSuffix(modifier);
        return $"{statName}: {sign}{FormatValue(modifier.Value)}{suffix}";
    }

    private string GetModifierStatName(ItemStatModifier modifier)
    {
        switch (modifier.Target)
        {
            case ItemStatTarget.Primary:
                return _statsConfig != null
                    ? _statsConfig.GetPrimaryStatDisplayName(modifier.PrimaryStat)
                    : modifier.PrimaryStat.ToString();
            case ItemStatTarget.Secondary:
                return _statsConfig != null
                    ? _statsConfig.GetSecondaryStatDisplayName(modifier.SecondaryStat)
